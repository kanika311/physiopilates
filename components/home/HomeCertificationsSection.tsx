...
export default function HomeCertificationsSection() {
  return (
    <section className="px-4 py-16 md:py-24" style={{ backgroundColor: "#f5fcfc" }}>
      <div className="mx-auto max-w-6xl text-center">
        <div className="inline-flex rounded-full border border-amber-200/70 bg-white/90 px-6 py-2 shadow-sm">
        ...
        <h2
          className="mx-auto mt-6 max-w-3xl text-3xl font-semibold md:text-[2.65rem]"
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
          {TAGS.map((t) => (
            <span
              key={t}
              className="inline-flex items-center gap-2 rounded-full border border-amber-200/80 bg-white px-4 py-2 text-[13px] font-medium shadow-sm text-neutral-800"
            >
              <FiCheck className="shrink-0" style={{ color: GOLD }} aria-hidden /> {t}
            </span>
          ))}
        </div>
        ...
        <div className="mx-auto mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {STATS.map(({ Icon, value, label }) => (
            <div
              key={label}
              className="flex flex-col items-center rounded-[1.85rem] border border-neutral-100/90 bg-white px-6 py-10 shadow-[0_16px_48px_-32px_rgba(0,0,0,0.18)]"
            >
              <div
              ...
              <p className="mt-8 text-xl font-bold text-neutral-900 md:text-2xl">{value}</p>
              <p className="mt-2 text-center text-sm text-neutral-600">{label}</p>
            </div>
          ))}
        </div>
      </div>
